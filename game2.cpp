...
initwindow(1250,680,"first game");
 ...
 setcolor(WHITE);
        if(ch==80) {cursor_y+=120;
        if(cursor_y > 386) {cursor_y=130;}
        }
        if(ch==72) {cursor_y-=120;
        if(cursor_y < 130) {cursor_y=386;}
        }
        outtextxy(160,cursor_y,">");
...
int p=517,q=336,v1=0,v2=0,v3=0,num=0,n=0;
while(1)
{cleardevice();
char a[300];

sprintf(a,"loading...%d",num);
setcolor(YELLOW);
outtextxy(p-27,q, a);
setlinestyle(3,0,2);

setcolor(WHITE);
arc(p,q,30-v2,160-v2,92);
setcolor(BLUE);
arc(p,q,30-v1,160-v1,72);
if(num==100){
goto start;
}
v1+=rand()%10+5;
...
clearviewport();
     setcolor(WHITE);
        ...
        setcolor(BLUE);
        settextstyle(3,0,4);
        outtextxy(300,220,"exit");
        outtextxy(300,190,"go to level");
        ...
        if(ch == 13) {
            if(cursor_y==220) exit(0);
            else if(cursor_y==160) help();


           else if(cursor_y==190) select_level();

        }
        }

        if(ch==80) {cursor_y+=30;
        if(cursor_y > 220) {cursor_y=160;}
        }
        if(ch==72) {cursor_y-=30;
        if(cursor_y < 160) {cursor_y=220;}
        }
...
void check_finish()//extream level
{
        ...
        setcolor(MAGENTA);
        if(ch==80) {cursor_y+=60;
        if(cursor_y > 250) {cursor_y=130;}
        }
        if(ch==72) {cursor_y-=60;
        if(cursor_y < 130) {cursor_y=250;}
        }
        outtextxy(215,cursor_y,">");
...
if(snake_size<17){tail_check();}
 if(snake_size>17){ mate();}
       ...
       for(int i = 1;i < snake_size;i++)
    {
        if(snake_x[0] == snake_x[i] && snake_y[0] == snake_y[i])
        {
            ...
            settextstyle(6,0,8);
                ...
                delay(250);
             while(1){  clearviewport(); settextstyle(6,0,8);
            setcolor(BLUE);
        settextstyle(7,0,6);
    ...
    {if(snake_x[0] == 0||snake_y[0]==0  ||snake_y[0] == k||snake_x[0]==j)
         ...
         delay(250);
            ...
            for(int i = 0;i < snake_size;i++)
            {
                ...
                delay(250);
             while(1){  clearviewport(); settextstyle(6,0,8);
            setcolor(BLUE);snake_x[0]=20;snake_y[0]=50;
        settextstyle(7,0,6);
        outtextxy(200,130,"play again");
        outtextxy(200,190,"Main Menu");
        outtextxy(200,250,"Exit");
...
while(1)
 ...
 setcolor(RED);
        if(ch==80) {cursor_y+=60;
        if(cursor_y > 310) {cursor_y=130;}
        }
        if(ch==72) {cursor_y-=60;
        if(cursor_y < 130) {cursor_y=310;}
        }
       outtextxy(180,cursor_y,">");
    ...
    for(int i = 1;i < snake_size;i++)
    {
        if(snake_x[0] == snake_x[i] && snake_y[0] == snake_y[i])
        {
            ...
            for(int i = 0;i < snake_size;i++)
            {
                ...
                settextstyle(6,0,8);
                outtextxy(50,200,"game over");
                delay(200);clearviewport();
             while(1){   settextstyle(6,0,8);
            setcolor(MAGENTA);
        settextstyle(7,0,5);
